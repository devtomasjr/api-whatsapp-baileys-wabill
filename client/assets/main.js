...
let formatData = () => {
	...
	if (dataIsValid) {
		...
		toast("error! dados inválidos", "danger")
		editData()
	}
}
...
let previewMessage = () => {
	if (!dataIsFormatted) {
		toast("Formate os dados primeiro", "danger")
	} else {
		formatData()
...
// var array = ['some', 'array', 'containing', 'words'];
	var interval = Math.floor(Math.random() * (100000 - 20000)); // how much time should the delay between two iterations be (in milliseconds)?
	...
	formattedData.forEach(function (e, i) {
		if (i !== 0) {
		  promise = promise.then(function () {
		    ...
		    console.log(`Enviar mensagens para ${e[2]}`)
		    ...
		    fetch("https://api-whatsapp-baileys-wabill-production.up.railway.app/send-message", {
				...
				.then(res => {
				  ...
				  if (res.status) {
				  	...
				  	toast("error! por favor abra o console", "danger")
					console.log("Falha na busca! err:", res)
				  }
				}).catch(err => {
					console.log("Falha na busca! err:", err)
					updateStatus(i - 1, "failed")
			  	toast("error! por favor abra o console", "danger")
				})
