...
app.get("/escanear", (req, res) => {
  res.sendFile("./client/qr-code.html", {
    root: __dirname,
  })
})
...
app.post("/send-message", async (req, res) => {
  ...
  if (connected) {
    wa.onWhatsApp(number)
      .then(data => {
        if (data[0]?.jid) {
          sendMessageWTyping(wa, message, data[0].jid)
            .then((result) => {
              res.status(200).json({
                status: true,
                response: result,
          ...
          res.status(500).json({
            status: false,
            response: `O número ${number} não está registrado.`,
          })
        }
      })
      .catch(async err => {
        ...
        if (err?.output?.statusCode === DisconnectReason.connectionClosed) {
          console.log('sim erro ')
        }
      })
  } else {
    res.status(500).json({
      status: false,
      response: `O WhatsApp ainda não está conectado.`,
    })
  }
})

server.listen(port, () => {
  console.log(`O aplicativo é executado em http://localhost:${port}`)
})